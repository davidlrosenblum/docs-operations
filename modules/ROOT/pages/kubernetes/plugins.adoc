...
The example below would allow only routines from apoc.math and apoc.cypher.
For example:
+
[source, yaml]
----
...
----
+
. Under `apoc_config`, configure the APOC settings that you want.  These behave if they were in an apoc.conf file. for example:
+
[source, yaml]
----
apoc_config:
...
. Under `env`, set the `NEO4J_PLUGINS` variable to `'["apoc"]' .  This would cause the image to load the apoc library.  for example:
+
[source, yaml]
----
env:
